...
%         fprintf('Difference between query image and database image %d\n', i);
    ...
    %Store precision values in result vector
    result((z-x+1),1) = resultRGB;
    result((z-x+1),2) = resultHSV;
    result((z-x+1),3) = resultCNN;
    result((z-x+1),4) = resultMSE;
    result((z-x+1),5) = resultSSIM;
